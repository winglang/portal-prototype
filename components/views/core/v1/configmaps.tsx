...
const BalancedBadge = ({ children, color }: { children: React.ReactNode, color: string }) => (
  <span className={`px-2 py-1 rounded-full ${color} text-xs font-semibold`}>{children}</span>
);
                ...
                <Tooltip>
                  <TooltipTrigger>
                    <div className="flex items-center gap-2 bg-gray-100 p-3 rounded-lg">
                      ...
                      <span className="font-semibold text-gray-800">Immutable:</span>
              ...
              {showMetadata && (
                <div className="pl-6 space-y-2 animate-fadeIn">
                  {obj.metadata.namespace && (
                    <div>
                      Namespace:{' '}
                      <BalancedBadge color="bg-blue-100 text-blue-800">{obj.metadata.namespace}</BalancedBadge>
                    </div>
                  )}
                  {obj.metadata.resourceVersion && (
                    <div>
                      Resource Version:{' '}
                      <BalancedBadge color="bg-green-100 text-green-800">{obj.metadata.resourceVersion}</BalancedBadge>
                    </div>
                  )}
                  {obj.metadata.uid && (
                    <div>
                      UID: <BalancedBadge color="bg-purple-100 text-purple-800">{obj.metadata.uid}</BalancedBadge>
                    </div>
                  )}
                  <div>
                    Created:{' '}
                    <BalancedBadge color="bg-yellow-100 text-yellow-800">{formatDate(obj.metadata.creationTimestamp)}</BalancedBadge>
                  </div>
              ...
              {obj.binaryData && Object.keys(obj.binaryData).length > 0 && (
                <>
                  ...
                  >
                    ...
                    {showBinaryData ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                  </Button>
                  {showBinaryData && (
                    <div className="pl-6 space-y-2 animate-fadeIn">
